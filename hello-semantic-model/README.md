...
## Run Release Health locally

Local development uses the Azure CLI identity to query the registered semantic
model. It does not require the Fabric portal iframe or Rayfin sign-in UI.
...
The Vite development server exposes a local-only DAX proxy. It accepts queries
only for semantic models registered in `fabric.yaml`, obtains short-lived tokens
through `az account get-access-token`, and never sends those tokens to the
browser. Deployed builds continue to use Fabric portal SSO and the embedded
postMessage proxy.

If the app reports an Azure CLI authentication error, run `az login` again and
reload the page.
