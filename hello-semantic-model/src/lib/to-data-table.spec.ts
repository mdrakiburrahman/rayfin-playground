...
describe("toDataTable", () => {
    ...
    describe("reorderDataTable", () => {
        it("reorders columns and aligned row values by column name", () => {
            const result = reorderDataTable(
                {
                    columns: [
                        { name: "ServiceGroup" },
                        { name: "Manager" },
                    ...
                    rows: [
                        ["ResourceProvider", "shilpimitra", 44],
                        ["AdxProxy", "anikam", 39],
                    ],
                },
                ["ServiceGroup", "Days", "Manager"],
            );
            ...
            expect(result).toEqual({
                ...
                rows: [
                    ["ResourceProvider", 44, "shilpimitra"],
                    ["AdxProxy", 39, "anikam"],
                ],
            });
        });

        it("fails when a requested column is missing", () => {
            expect(() =>
                reorderDataTable(
                    {
                        columns: [{ name: "ServiceGroup" }],
                        rows: [["AdxProxy"]],
                    },
                    ["Days"],
                ),
            ).toThrow('Data table column "Days" was not found.');
        });
    });
