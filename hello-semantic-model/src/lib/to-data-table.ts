...
export function reorderDataTable(
    ...
    const columnIndexes = new Map(
        dataTable.columns.map((column, index) => [column.name, index]),
    );
    const orderedIndexes = columnNames.map((columnName) => {
        const index = columnIndexes.get(columnName);
        if (index === undefined) {
            throw new Error(`Data table column "${columnName}" was not found.`);
        }
        return index;
    });

    return {
        columns: orderedIndexes.map((index) => dataTable.columns[index]!),
        rows: dataTable.rows.map((row) =>
            orderedIndexes.map((index) => row[index]),
        ),
    };
}
